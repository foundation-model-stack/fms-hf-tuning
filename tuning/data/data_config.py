...
def validate_data_config(dataconfig: DataConfig):
    ...
    if dataconfig.chat_template is not None:
        assert isinstance(dataconfig.chat_template, str), "chat_template should be a string"
    if dataconfig.chat_template_b64 is not None:
        assert isinstance(dataconfig.chat_template_b64, str), "chat_template_b64 should be a string"
...
def load_and_validate_data_config(data_config_file: str) -> DataConfig:
    ...
    data_config = DataConfig(dataprocessor=dataprocessor, datasets=datasets, chat_template=chat_template)
    return data_config
