...
class DataPreProcessor:
    ...
    def _process_dataset_configs(
        ...
        for d in dataset_configs:
            if d.sampling:
                sampling_probabilities.append(d.sampling)
        ...
        if len(sampling_probabilities) > 0:
            ...
            logging.info(
                "Sampling is not specified given datasets will be concatenated"
            )
        ...
        if sample_datasets:
            ...
            for k, v in final_datasets.items():
                combined = datasets.concatenate_datasets(v)
                final_datasets[k] = combined
