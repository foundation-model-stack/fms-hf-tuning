...
class Stage2(Scene):
    def construct(self):
        ...
        for i, rect in enumerate(model_base):
            ...
            cpu_target = (
                Rectangle(height=0.46 / 4, width=0.46 / 3)
            ...
            if i == 0:
                cpu_target.next_to(
                    cpu_left_col_base[0].get_corner(DOWN + LEFT),
                    buff=0.02,
                    direction=UP,
                )
        ...
        checkpoint = Group(checkpoint_rect, checkpoint_text).arrange(
            DOWN, aligned_edge=DOWN, buff=0.4
        )
        ...
        self.play(
            Write(checkpoint_text, run_time=1), Create(checkpoint_rect, run_time=1)
        )
