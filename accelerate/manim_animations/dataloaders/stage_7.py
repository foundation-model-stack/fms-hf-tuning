...
class Stage7(Scene):
    def construct(self):
        ...
        columns = [
            VGroup(
                *[Rectangle(height=0.25, width=0.25, color=colors[j]) for i in range(8)]
        ...
        gpu_1 = (
            Group(
                Rectangle(color="white", height=1.02, width=0.98),
        ...
        for i, row_data in enumerate(dataset_recs):
            ...
            for j, indiv_data in enumerate(row_data):
                dataset_target = (
                    Rectangle(height=0.46 / 4, width=0.46 / 2)
                ...
                if j % 8 == 0:
                    ...
                    dataset_target.target.next_to(
                        samplers[0].get_corner(DOWN + LEFT),
                        buff=0.0125,
                        direction=RIGHT + UP,
                    )
                    dataset_target.target.set_x(dataset_target.target.get_x())
                    dataset_target.target.set_y(
                        dataset_target.target.get_y() + (0.25 * i)
                    )
                elif j % 4 == 0:
                    ...
                    dataset_target.target.set_x(dataset_target.target.get_x())
                    dataset_target.target.set_y(
                        dataset_target.target.get_y() + 0.125 + (0.25 * i)
                    )
                else:
                    dataset_target.target.next_to(
                        old_target,
                ...
                first_animations.append(
                    indiv_data.animate(run_time=0.5).set_stroke(current_color)
                )
            ...
            for j, indiv_data in enumerate(row):
                ...
                if j % 8 == 0:
                    ...
                    indiv_data.target.next_to(
                        gpus[abs(idx)].get_corner(UP + LEFT),
                        buff=0.01,
