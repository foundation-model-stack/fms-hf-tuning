...
class Stage2(Scene):
    def construct(self):
        ...
        columns = [
            VGroup(
                *[Rectangle(height=0.25, width=0.25, color="green") for i in range(8)]
            ).arrange(RIGHT, buff=0)
            for j in range(4)
        ]
        dataset_recs = VGroup(*columns).arrange(UP, buff=0)
        dataset_text = Text("Dataset", font_size=24)
        dataset = Group(dataset_recs, dataset_text).arrange(
            DOWN, buff=0.5, aligned_edge=DOWN
        )
        ...
        dataloader = Group(
            Rectangle(color="red", height=2, width=2), Text("DataLoader", font_size=24)
        ).arrange(DOWN, buff=0.5, aligned_edge=DOWN)

        sampler = Group(
            Rectangle(color="blue", height=1, width=1), Text("Sampler", font_size=12)
        ).arrange(DOWN, buff=0.25, aligned_edge=DOWN)
        ...
        gpu_1 = (
            Group(
                Rectangle(color="white", height=1, width=1), Text("GPU 1", font_size=12)
            )
            .arrange(DOWN, buff=0.25, aligned_edge=DOWN)
            .move_to([4, 2, 0])
        )
        gpu_2 = (
            Group(
                Rectangle(color="white", height=1, width=1), Text("GPU 2", font_size=12)
            )
            .arrange(DOWN, buff=0.25, aligned_edge=DOWN)
            .move_to([4, 0.5, 0])
        )
        gpu_3 = (
            Group(
                Rectangle(color="white", height=1, width=1), Text("GPU 3", font_size=12)
            )
            .arrange(DOWN, buff=0.25, aligned_edge=DOWN)
            .move_to([4, -1, 0])
        )
        gpu_4 = (
            Group(
                Rectangle(color="white", height=1, width=1), Text("GPU 4", font_size=12)
            )
            .arrange(DOWN, buff=0.25, aligned_edge=DOWN)
            .move_to([4, -2.5, 0])
        )
        ...
        for i, data in enumerate(dataset_recs[-1]):
            ...
            dataset_target = (
                Rectangle(height=0.46 / 2, width=0.46 / 2)
                .set_stroke(width=0.0)
                .set_fill(current_color, opacity=0.7)
            )
            ...
            if i % 2 == 0:
                ...
                dataset_target.target.next_to(
                    sampler, buff=buff, direction=UP, aligned_edge=LEFT
                )
            else:
                dataset_target.target.next_to(
                    old_target,
            ...
            first_animations.append(
                data.animate(run_time=0.5).set_stroke(current_color)
            )
