...
def lambda_test(
    ...
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer, lr_lambda=lambda n: 1 - n / 10
    )
    model, optimizer, scheduler = accelerator.prepare(model, optimizer, scheduler)
    ...
    expected_lr = (
        ...
        - (
            num_processes
            if (step_scheduler_with_optimizer and not split_batches)
            else 1
        )
        / 10
    )
...
def accumulation_test(num_processes: int = 2):
    ...
    for num_steps in steps:
        plugin = GradientAccumulationPlugin(
            num_steps=num_steps, adjust_scheduler=num_steps > 1
        )
        accelerator = Accelerator(gradient_accumulation_plugin=plugin)
        model = torch.nn.Linear(2, 4)
        optimizer = torch.optim.AdamW(model.parameters(), lr=10.0)
        scheduler = get_linear_schedule_with_warmup(
            optimizer=optimizer, num_warmup_steps=0, num_training_steps=20
        )
...
class SchedulerTester(unittest.TestCase):
    def test_lambda_scheduler_steps_with_optimizer_single_process(self):
        ...
        debug_launcher(
            partial(lambda_test, num_processes=1, split_batches=True), num_processes=1
        )
    ...
    def test_one_cycle_scheduler_steps_with_optimizer_single_process(self):
        ...
        debug_launcher(
            partial(one_cycle_test, num_processes=1, split_batches=True),
            num_processes=1,
        )

    def test_lambda_scheduler_not_step_with_optimizer_single_process(self):
        debug_launcher(
            partial(lambda_test, num_processes=1, step_scheduler_with_optimizer=False),
            num_processes=1,
        )
    ...
    def test_one_cycle_scheduler_not_step_with_optimizer_single_process(self):
        debug_launcher(
            partial(
                one_cycle_test, num_processes=1, step_scheduler_with_optimizer=False
            ),
            num_processes=1,
        )
