    ...
    def __post_init__(self):
        ...
        if self.backend == "TE":
            ...
            if self.opt_level not in get_args(OptLevel):
                raise ValueError(
                    f"`optimization_level` must be one of {' or '.join(get_args(OptLevel))}"
                )
...
class FullyShardedDataParallelPlugin:
    ...
    mixed_precision_policy: Optional[
        Union[dict, "torch.distributed.fsdp.MixedPrecision"]
    ] = field(
        default=None,
        metadata={
    ...
    use_orig_params: bool = field(
        default=None,
        metadata={
            "help": "Whether to use the original parameters for the optimizer. Defaults to `False`"
        },
    )
    ...
    def set_mixed_precision(
        ...
        if override or self.mixed_precision_policy is None:
            ...
            missing_keys = [
                ...
                if k not in self.mixed_precision_policy
            ]
            invalid_values = [
                k
...
class FullyShardedDataParallelPlugin2:
    ...
    offload_policy: Optional[
        Union[dict, "torch.distributed._composable.OffloadPolicy"]
    ] = field(
        default=None,
        metadata={
    ...
    mp_policy: Optional[
        Union[dict, "torch.distributed._composable.MixedPrecisionPolicy"]
    ] = field(
        default=None,
        metadata={
    ...
    def __post_init__(self):
        ...
        if self.reshard_after_forward is None:
            self.reshard_after_forward = (
                str_to_bool(
                    os.environ.get(env_prefix + "RESHARD_AFTER_FORWARD", "True")
                )
                == 1
            )
        ...
        self.torch_device_mesh = init_device_mesh(
            ...
            mesh_dim_names=(dp_mesh_dim_name, fsdp_mesh_dim_name),
        )
    ...
    def set_offload_policy(self, pin_memory=None):
        ...
        if self.offload_policy is None:
            fsdp2_cpu_offload = (
                str_to_bool(os.environ.get(env_prefix + "CPU_OFFLOAD", "False")) == 1
            )
            if fsdp2_cpu_offload:
                pin_memory = (
                    str_to_bool(
                        os.environ.get(env_prefix + "CPU_OFFLOAD_PIN_MEMORY", "False")
                    )
                    == 1
                )
    ...
    def set_mp_policy(
        ...
        if self.mp_policy is None:
            param_dtype = mixed_precision_mapping.get(
                os.environ.get(env_prefix + "MP_PARAM_DTYPE", param_dtype), None
            )
            ...
            if not cast_forward_inputs:
                cast_forward_inputs = (
                    str_to_bool(
                        os.environ.get(env_prefix + "CAST_FORWARD_INPUTS", "False")
                    )
                    == 1
                )
...
class TorchTensorParallelPlugin:
    ...
    def __post_init__(self):
        self.tp_size = (
            self.tp_size
            if os.environ.get("TP_SIZE", "1") == "1"
            else int(os.environ.get("TP_SIZE", "1"))
        )
        if self.tp_size == 1:
        ...
        device = "cuda"  # support for other devices has to be investigated
        self.torch_device_mesh = init_device_mesh(
            device, (self.tp_size,), mesh_dim_names=(mesh_dim_name,)
        )
