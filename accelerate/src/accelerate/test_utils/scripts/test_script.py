...
def generate_baseline_dataloader(
    train_set, generator, batch_size, use_seedable_sampler=False
):
...
def process_execution_check():
    ...
    if accelerator.is_main_process:
        ...
        try:
            assert text.startswith(
                "Currently in the main process\n"
            ), "Main process was not first"
            if num_processes > 1:
                assert text.endswith(
                    "Now on another process\n"
                ), "Main process was not first"
            assert (
    ...
    if accelerator.is_main_process:
        assert (
            result == "Printing from the main process 0"
        ), f"{result} != Printing from the main process 0"
    else:
    ...
    if accelerator.is_last_process:
        assert (
            f.getvalue().rstrip()
            == f"Printing from the last process {accelerator.state.num_processes - 1}"
        )
    else:
        assert f.getvalue().rstrip() == ""
    f.truncate(0)
    ...
    for process_idx in range(num_processes):
        with contextlib.redirect_stdout(f):
            accelerator.on_process(print_on, process_index=process_idx)(
                accelerator.state, process_idx
            )
        if accelerator.process_index == process_idx:
            assert (
                f.getvalue().rstrip()
                == f"Printing from process {process_idx}: {accelerator.process_index}"
            )
        else:
            assert f.getvalue().rstrip() == ""
        f.truncate(0)
...
def dl_preparation_check():
    ...
    dl = prepare_data_loader(
        dl, state.device, state.num_processes, state.process_index, put_on_device=True
    )
    result = []
    for batch in dl:
    ...
    assert torch.equal(
        result.cpu(), torch.arange(0, length).long()
    ), "Wrong non-shuffled dataloader result."
...
def check_seedable_sampler():
    ...
    ), "Did not obtain the same items with the same seed and epoch."
...
def check_seedable_sampler_with_data_seed():
    ...
    assert not torch.allclose(
        original_items, new_items
    ), "Obtained the same items with different data seed."
...
def mock_training(length, batch_size, generator, use_seedable_sampler=False):
    ...
    train_dl = generate_baseline_dataloader(
        train_set, generator, batch_size, use_seedable_sampler
    )
...
def training_check(use_seedable_sampler=False):
    ...
    assert are_the_same_tensors(
        old_model.a
    ), "Did not obtain the same model on both processes."
    assert are_the_same_tensors(
        old_model.b
    ), "Did not obtain the same model on both processes."
    ...
    assert torch.allclose(
        old_model.a, model.a
    ), "Did not obtain the same model on CPU or distributed training."
    assert torch.allclose(
        old_model.b, model.b
    ), "Did not obtain the same model on CPU or distributed training."
    ...
    dataloader_config = DataLoaderConfiguration(
        split_batches=True, use_seedable_sampler=use_seedable_sampler
    )
    ...
    if (
        ...
        accelerator = Accelerator(
            mixed_precision="fp16", dataloader_config=dataloader_config
        )
        train_dl = generate_baseline_dataloader(
            train_set, generator, batch_size, use_seedable_sampler
        )
    ...
    if torch.cuda.is_available():
        ...
        model_with_fp32_wrapper = accelerator.unwrap_model(
        ...
        input_tensor = torch.Tensor([1, 2]).to(
            dtype=torch.float16, device=accelerator.device
        )
    ...
    if is_ipex_available():
        ...
        accelerator = Accelerator(
            mixed_precision="bf16", cpu=True, dataloader_config=dataloader_config
        )
        train_dl = generate_baseline_dataloader(
            train_set, generator, batch_size, use_seedable_sampler
        )
    ...
    if is_xpu_available():
        ...
        accelerator = Accelerator(
            mixed_precision="bf16", cpu=False, dataloader_config=dataloader_config
        )
        train_dl = generate_baseline_dataloader(
            train_set, generator, batch_size, use_seedable_sampler
        )
        ...
        assert torch.allclose(
            old_model.a, model.a
        ), "Did not obtain the same model on XPU or distributed training."
        assert torch.allclose(
            old_model.b, model.b
        ), "Did not obtain the same model on XPU or distributed training."
...
def test_split_between_processes_dataset(datasets_Dataset):
    state = AcceleratorState()
    data = datasets_Dataset.from_list(
        [dict(k=v) for v in range(2 * state.num_processes)]
    )
    ...
    data = datasets_Dataset.from_list(
        [dict(k=v) for v in range(2 * state.num_processes - 1)]
    )
    with state.split_between_processes(data, apply_padding=False) as results:
    ...
    if state.num_processes in (1, 2, 4):
        ...
        with state.split_between_processes(data) as results:
            if state.process_index == 0:
                ...
                assert (
                    results["a"] == data_copy["a"][-2:]
                ), f'Expected: {data_copy["a"][-2]}, Actual: {results["a"]}'
            if state.process_index == 0:
...
def test_split_between_processes_tensor():
    ...
    if state.num_processes > 1:
        ...
        with state.split_between_processes(data) as results:
            if state.process_index == 0:
                assert torch.allclose(
                    results, torch.tensor([0, 1, 2, 3]).to(state.device)
                )
            else:
                assert torch.allclose(
                    results, torch.tensor([4, 5, 6, 7]).to(state.device)
                )
    state.wait_for_everyone()
