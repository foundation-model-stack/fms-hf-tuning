...
from mixing.utils.loss import load_balancing as load_balancing_loss_func
...
def make_moe_modules(moe_class_partial):
    ...
    class MoeModules(nn.Module):
        ...
        def forward(self, hidden_states: torch.Tensor, *args, **kwargs) -> torch.Tensor:
            ...
            if self.config.output_router_logits: self.router_logits = router_logits
...
def make_moe_attention_modules(attention_class):

    class MixLlamaAttention(attention_class):
...
class MixLlamaDecoderLayer(LlamaDecoderLayer):
    ...
    def __init__(self, config: MixLlamaConfig, layer_idx: int):
        ...
        if self.moe_attention:
            ...
            self.self_attn = make_moe_attention_modules(
