...
def temporarily_disable_deepspeed_zero3():
    ...
    if is_deepspeed_zero3_enabled():
        ...
        unset_hf_deepspeed_config()
        yield
        set_hf_deepspeed_config(HfDeepSpeedConfig(ds_config))
    else:
        print("ds not enabled")
        yield
...
def main(args: FlatArguments, tc: TokenizerConfig):
    ...
    #     # we load moe module only on rank 0
    #     # and then redistribute it over other ranks
    #     # this gives cpu ram efficient loading effect
                        ...
                        if accelerator.is_local_main_process:
                            if fast_moe_config is not None:
                                ...
                                if not os.path.exists(converted):
                                    ...
                                    logger.info(
                                        f"accelerator state saved at {converted}"
                                    )
                                    logger.info(
                                    ...
                                    logger.info(
                                        f"converted model is saved at {converted}"
                                    )
