...
class FastCrossEntropyLoss(torch.nn.CrossEntropyLoss):
    ...
    def forward(self, input, target):
        loss = Fast_CrossEntropyLoss.apply(input, target)
...
def FastForCausalLMLoss(
    ...
    **kwargs
    ...
    assert (
        ignore_index == -100
    ), "FastForCausalLMLoss currently supports only hardcoded ignore index -100."
    loss = Fast_CrossEntropyLoss.apply(shift_logits, shift_labels)
    if reduction == "sum":
