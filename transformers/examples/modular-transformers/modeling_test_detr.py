...
class TestDetrEncoder(TestDetrPreTrainedModel):
    ...
    def __init__(self, config: TestDetrConfig):
        ...
        self.layers = nn.ModuleList(
            [TestDetrEncoderLayer(config) for _ in range(config.encoder_layers)]
        )
...
class TestDetrDecoder(TestDetrPreTrainedModel):
    ...
    def __init__(self, config: TestDetrConfig):
        ...
        self.dropout = config.dropout
        self.layers = nn.ModuleList(
            [TestDetrDecoderLayer(config) for _ in range(config.decoder_layers)]
        )
        self.gradient_checkpointing = False
...
class TestDetrModel(TestDetrPreTrainedModel):
    ...
    @replace_return_docstrings(
        output_type=TestDetrModelOutput, config_class=_CONFIG_FOR_DOC
    )
    def forward(
        self,
