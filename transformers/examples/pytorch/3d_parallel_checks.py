...
def main():
    ...
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
        ...
        world_mesh = DeviceMesh(
            device_type="cuda", mesh=mesh, mesh_dim_names=("dp", "tp", "cp")
        )
        ...
        wandb.init(
            ...
            name="llama_tp1_dp1_nondist"
            if model_name == "unsloth/Llama-3.2-1B"
            else "tp1_dp1_nondist",
        )
    ...
    if dist.is_initialized() and dp_mesh.size() > 1:
        # FSDP1
        model = FSDP(
            model, device_mesh=dp_mesh, sharding_strategy=ShardingStrategy.NO_SHARD
        )
        # FSDP2
    ...
    ), "No parameters found in model. Probably DDP bug.."
    assert (
    ...
    while step < num_train_steps:
        for batch in dataloader:
            ...
            with sdpa_kernel(
                sdpa_backend
            ):  # TODO: ideally move this to attention implementation
                cp_context = (
                    nullcontext()
                    if cp_mesh.size() == 1
                        ...
                        if param.grad is not None and isinstance(param.grad, DTensor):
                            if param.grad.placements[0].is_shard():
                                sanity_check_tensor_sync(
                                    param.grad, tp_mesh, not_sync=True
                                )
                ...
                for i, (group1, group2) in enumerate(zip(state1, state2)):
                    for key in group1:
                        if key != "params":  # Skip comparing the params list
                            ...
                            ), f"Param group mismatch in param_groups[{i}][{key}]"
...
def sanity_check_tensor_sync(
    ...
    for i in range(1, world_size):
        try:
            torch.testing.assert_close(
                gathered_tensors[0], gathered_tensors[i], rtol=rtol, atol=atol
            )
...
def check_params_sync(model_params, original_params):
    ...
    for mp, op in zip(model_params, original_params):
        ...
        if not torch.allclose(mp.data, op.data, rtol=0, atol=0):
            raise RuntimeError(
                f"Parameters out of sync: model param {mp.data} != original param {op.data}"
            )
    return True
