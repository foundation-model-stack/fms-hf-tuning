    ...
    for layer_name_vision in state_dict_keys:
        if (
            ...
            if "weight" in layer_name_vision:
                ...
                state_dict[
                    layer_name_vision.replace("qkv.weight", "key.weight")
                ] = in_proj_weight[:hidden_size, :]
                state_dict[
                    layer_name_vision.replace("qkv.weight", "query.weight")
                ] = in_proj_weight[hidden_size : hidden_size * 2, :]
                state_dict[
                    layer_name_vision.replace("qkv.weight", "value.weight")
                ] = in_proj_weight[-hidden_size:, :]
            elif "bias" in layer_name_vision:
                in_proj_bias = state_dict.pop(layer_name_vision)
                state_dict[
                    layer_name_vision.replace("qkv.bias", "key.bias")
                ] = in_proj_bias[:hidden_size]
                state_dict[
                    layer_name_vision.replace("qkv.bias", "query.bias")
                ] = in_proj_bias[hidden_size : hidden_size * 2]
                state_dict[
                    layer_name_vision.replace("qkv.bias", "value.bias")
                ] = in_proj_bias[-hidden_size:]
    ...
    for layer_name_text in state_dict_keys:
        if (
            ...
            if "weight" in layer_name_text:
                ...
                state_dict[
                    layer_name_text.replace("in_proj_bias", "q_proj.bias")
                ] = in_proj_bias[:hidden_size]
                state_dict[
                    layer_name_text.replace("in_proj_bias", "k_proj.bias")
                ] = in_proj_bias[hidden_size : hidden_size * 2]
                state_dict[
                    layer_name_text.replace("in_proj_bias", "v_proj.bias")
                ] = in_proj_bias[-hidden_size:]
...
def read_in_q_k_v_decoder(state_dict, config):
    for layer_num in range(config.decoder_num_layers):
        ...
        # next, add query, keys and values (in that order) to the state dict
        state_dict[
            f"decoder.layers.{layer_num}.self_attn.query.weight"
        ...
        state_dict[
            f"decoder.layers.{layer_num}.self_attn.value.weight"
        ] = in_proj_weight[-embed_dim:, :]
