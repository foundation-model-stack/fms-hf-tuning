...
def get_kernel_predictor_key_mapping(
    ...
    for i in range(config.kernel_predictor_num_blocks):
        mapping[
            f"{old_prefix}.residual_convs.{i}.1.weight_g"
        ] = f"{new_prefix}.resblocks.{i}.conv1.weight_g"
        mapping[
            f"{old_prefix}.residual_convs.{i}.1.weight_v"
        ] = f"{new_prefix}.resblocks.{i}.conv1.weight_v"
        mapping[
            f"{old_prefix}.residual_convs.{i}.1.bias"
        ] = f"{new_prefix}.resblocks.{i}.conv1.bias"

        mapping[
            f"{old_prefix}.residual_convs.{i}.3.weight_g"
        ] = f"{new_prefix}.resblocks.{i}.conv2.weight_g"
        mapping[
            f"{old_prefix}.residual_convs.{i}.3.weight_v"
        ] = f"{new_prefix}.resblocks.{i}.conv2.weight_v"
        mapping[
            f"{old_prefix}.residual_convs.{i}.3.bias"
...
def get_key_mapping(config: UnivNetConfig):
    ...
    for i in range(len(config.resblock_stride_sizes)):
        ...
        mapping[
            f"res_stack.{i}.convt_pre.1.weight_g"
        ] = f"resblocks.{i}.convt_pre.weight_g"
        mapping[
            f"res_stack.{i}.convt_pre.1.weight_v"
        ...
        for j in range(len(config.resblock_dilation_sizes[i])):
            ...
            mapping[
                f"res_stack.{i}.conv_blocks.{j}.1.bias"
