...
def read_in_q_k_v_decoder(state_dict, config):
    ...
    for idx in range(config.decoder_layers):
        ...
        # next, add query, keys and values (in that order) to the state dict
        state_dict[
            f"model.decoder.layers.{idx}.self_attn.query.weight"
        ] = in_proj_weight[:hidden_size, :]
        ...
        state_dict[
            f"model.decoder.layers.{idx}.self_attn.value.weight"
        ] = in_proj_weight[-hidden_size:, :]
        ...
        # next, add query, keys and values (in that order) to the state dict
        state_dict[
            f"model.decoder.layers.{idx}.encoder_attn_text.query.weight"
        ] = in_proj_weight[:hidden_size, :]
        state_dict[
            f"model.decoder.layers.{idx}.encoder_attn_text.query.bias"
        ] = in_proj_bias[:hidden_size]

        state_dict[
            f"model.decoder.layers.{idx}.encoder_attn_text.key.weight"
        ] = in_proj_weight[hidden_size : hidden_size * 2, :]
        state_dict[
            f"model.decoder.layers.{idx}.encoder_attn_text.key.bias"
        ] = in_proj_bias[hidden_size : hidden_size * 2]

        state_dict[
            f"model.decoder.layers.{idx}.encoder_attn_text.value.weight"
        ] = in_proj_weight[-hidden_size:, :]
        state_dict[
            f"model.decoder.layers.{idx}.encoder_attn_text.value.bias"
        ] = in_proj_bias[-hidden_size:]
