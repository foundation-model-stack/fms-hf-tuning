...
def read_in_k_v(state_dict, config):
    ...
    for i in range(config.num_encoder_blocks):
        for j in range(config.depths[i]):
            ...
            state_dict[
                f"pvt_v2.encoder.layers.{i}.blocks.{j}.attention.key.weight"
            ] = kv_weight[: config.hidden_sizes[i], :]
            state_dict[
                f"pvt_v2.encoder.layers.{i}.blocks.{j}.attention.key.bias"
            ...
            state_dict[
                f"pvt_v2.encoder.layers.{i}.blocks.{j}.attention.value.bias"
            ] = kv_bias[config.hidden_sizes[i] :]
