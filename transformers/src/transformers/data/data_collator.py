...
class DataCollatorForLanguageModeling(DataCollatorMixin):
    ...
    def tf_mask_tokens(
        ...
        indices_replaced = (
            self.tf_bernoulli(input_shape, self.mask_replace_prob) & masked_indices
        )
        ...
        indices_random = (
            self.tf_bernoulli(input_shape, random_replace_prob_scaled)
            & masked_indices
            & ~indices_replaced
        )
    ...
    def torch_mask_tokens(
        ...
        # mask_replace_prob% of the time, we replace masked input tokens with tokenizer.mask_token ([MASK])
        indices_replaced = (
            torch.bernoulli(torch.full(labels.shape, self.mask_replace_prob)).bool()
            & masked_indices
        )
    ...
    def numpy_mask_tokens(
        ...
        random_replace_prob_scaled = self.random_replace_prob / remaining_prob
        indices_random = (
            np.random.binomial(1, random_replace_prob_scaled, size=labels.shape).astype(
                bool
            )
            & masked_indices
            & ~indices_replaced
