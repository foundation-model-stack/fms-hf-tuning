...
class VisualQuestionAnsweringPipelineTests(unittest.TestCase):
    ...
    def test_small_model_pt_image_list(self):
        ...
        outputs = vqa_pipeline(
            image=images, question="How many cats are there?", top_k=1
        )
        self.assertEqual(
            outputs,
            [
    ...
    def test_small_model_pt_dataset(self):
        ...
        dataset = load_dataset(
            "hf-internal-testing/dummy_image_text_data", split="train[:2]"
        )
        question = "What's in the image?"

        outputs = vqa_pipeline(
            image=KeyDataset(dataset, "image"), question=question, top_k=1
        )
        self.assertEqual(
            outputs,
            [
