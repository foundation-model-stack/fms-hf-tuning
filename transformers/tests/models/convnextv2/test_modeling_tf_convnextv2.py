...
class TFConvNextV2ModelTest(TFModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    all_model_classes = (
        (TFConvNextV2Model, TFConvNextV2ForImageClassification)
        if is_tf_available()
        else ()
    )
    pipeline_model_mapping = (
        {
            "feature-extraction": TFConvNextV2Model,
            "image-classification": TFConvNextV2ForImageClassification,
        }
        if is_tf_available()
        else {}
    )
...
class TFConvNextV2ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_image_classification_head(self):
        model = TFConvNextV2ForImageClassification.from_pretrained(
