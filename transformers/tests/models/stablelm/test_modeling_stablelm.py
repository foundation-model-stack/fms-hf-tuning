...
class StableLmModelTest(
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=StableLmConfig, hidden_size=37
        )
...
class StableLmModelIntegrationTest(unittest.TestCase):
    ...
    def test_model_stablelm_3b_4e1t_logits(self):
        ...
        model = StableLmForCausalLM.from_pretrained("stabilityai/stablelm-3b-4e1t").to(
            torch_device
        )
        model.eval()
        ...
        EXPECTED_MEAN = torch.tensor([[2.7146, 2.4245, 1.5616, 1.4424, 2.6790]]).to(
            torch_device
        )
        torch.testing.assert_close(
            output.mean(dim=-1), EXPECTED_MEAN, rtol=1e-4, atol=1e-4
        )
        ...
        torch.testing.assert_close(
            output[0, 0, :30], EXPECTED_SLICE, rtol=1e-4, atol=1e-4
        )
    ...
    def test_model_tiny_random_stablelm_2_logits(self):
        ...
        EXPECTED_MEAN = torch.tensor(
            [[-2.7196, -3.6099, -2.6877, -3.1973, -3.9344]]
        ).to(torch_device)
        torch.testing.assert_close(
            output.mean(dim=-1), EXPECTED_MEAN, rtol=1e-4, atol=1e-4
        )
